...
async function main() {
  ...
  const equals = (expect, actual, message) => {
    if (typeof expect !== typeof actual) {
      throw Error('Type mismatch');
    }

    if (typeof expect === 'string' && expect.toLowerCase() !== actual.toLowerCase()) {
      throw Error(message);
    }
  }
  ...
  const unitroller = Cointroller.attach(addresses.Cointroller);
  const rifiAddress = await unitroller.getRifiAddress();

  required(rifiAddress, 'Cointroller: RIFI address not set');
  equals(config.RIFI, rifiAddress, 'Cointroller: RIFI address not match');
    ...
    //       if (!address) {
    //         throw Error(`${symbol}: Model config not found`);
    //       }

    //       console.log("Creating RBep20Delegator");
    //       const rBep20Delegator = RBep20Delegator.attach(addresses[symbol]);

    //       const modelAddress = await rBep20Delegator.interestRateModel();

    //       expect(modelAddress === address, `${symbol}: Model address mismatch.`);
    //     });
