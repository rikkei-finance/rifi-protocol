...
async function main() {
  ...
  const [
    ...
    ethers.getContractFactory("RMatic"),
  ...
  try {
    ...
    if (config.rMatic) {
      ...
      await runWithProgressCheck("rMatic", async () => {
        let modelAddress = address;
        if (!modelAddress) {
          const interestRateModel = await deployJumpRateModel(params);
          console.log(
            `rMatic InterestRateModel address at: ${explorer}/address/${interestRateModel.address}`
          );
          modelAddress = interestRateModel.address;
          config.rMatic.interestRateModel.address = modelAddress;
        }
        ...
        const rMatic = await RMatic.deploy(
        ...
        await rMatic.deployed();

        console.log(`rMatic address at: ${explorer}/address/${rMatic.address}`);

        addresses[symbol] = rMatic.address;

        await hre.run("verify:verify", {
          address: rMatic.address,
          constructorArguments: [
            addresses.Cointroller,
            modelAddress,
            web3.utils.toWei(initialExchangeRateMantissa, "ether"),
      ...
      await runWithProgressCheck("rMatic: unitroller._supportMarket", async () => {
        let transaction = await unitroller._supportMarket(addresses[symbol]);
        console.log(
          `rMatic Unitroller._supportMarket transaction: ${explorer}/tx/${transaction.tx}`
        );
      });

      await runWithProgressCheck("rMatic: priceOracle.setOracleData", async () => {
        transaction = await priceOracle.setOracleData(
          addresses[symbol],
          chainlink[underlying.symbol].address
        );
        console.log(`rMatic priceOracle.setOracleData transaction: ${explorer}/tx/${transaction.tx}`);
      });

      await runWithProgressCheck("rMatic: unitroller._setCollateralFactor", async () => {
        transaction = await unitroller._setCollateralFactor(
          addresses[symbol],
          web3.utils.toWei(config.rMatic.collateralFactor, "ether")
        );
        console.log(
          `rMatic Unitroller._setCollateralFactor transaction: ${explorer}/tx/${transaction.tx}`
        );
      });

      await runWithProgressCheck("rMatic: unitroller._setRifiSpeeds", async () => {
        transaction = await unitroller._setRifiSpeeds(
          [addresses[symbol]],
          [web3.utils.toWei(config.rMatic.rifiSupplySpeed, "ether")],
          [web3.utils.toWei(config.rMatic.rifiBorrowSpeed, "ether")]
        );
        console.log(`rMatic Unitroller._setRifiSpeeds transaction: ${explorer}/tx/${transaction.tx}`);
      });
      ...
      console.log("rMatic is not configured.");
    }
    ...
    if (!addresses.Maximillion && config.rMatic) {
      const maximillion = await Maximillion.deploy(addresses[config.rMatic.symbol]);
      ...
      await hre
        .run("verify:verify", {
          ...
          constructorArguments: [addresses[config.rMatic.symbol]],
        })
        .catch((e) => console.log(e.message));
    }
  } catch (e) {
