...
async function main() {
  ...
  const pendingAdmin = '0x631eC3da0980Eeb4d6656Fe2e478a32b4c8bf9f8';

  const unitroller = Unitroller.attach(addresses.Cointroller);

  await runWithProgressCheck(`Unitroller._setPendingAdmin`, async () => {
    const transaction = await unitroller._setPendingAdmin(pendingAdmin);
    console.log(
      `Unitroller._setPendingAdmin transaction: ${explorer}/tx/${transaction.hash}`
    );
  });

  const priceFeed = SimplePriceOracle.attach(addresses.PriceFeed);

  await runWithProgressCheck(`PriceFeed._setPendingAdmin`, async () => {
    const transaction = await priceFeed._setPendingAdmin(pendingAdmin);
    console.log(
      `PriceFeed._setPendingAdmin transaction: ${explorer}/tx/${transaction.hash}`
    );
  });
  ...
  try {
    if (config.rNative) {
      ...
      await runWithProgressCheck("rNative._setPendingAdmin", async () => {
        const rBinance = RBinance.attach(addresses[symbol]);
        const transaction = await rBinance._setPendingAdmin(pendingAdmin);
        console.log(
          `rNative._setPendingAdmin transaction: ${explorer}/tx/${transaction.hash}`
        );
      });
    } else {
    ...
    if (tokens.length > 0) {
      for (let [, token] of tokens) {
        ...
        await runWithProgressCheck(`${symbol}._setPendingAdmin`, async () => {
          const rBep20Delegator = RBep20Delegator.attach(addresses[symbol]);
          const transaction = await rBep20Delegator._setPendingAdmin(pendingAdmin);
          console.log(
            `${symbol}._setPendingAdmin transaction: ${explorer}/tx/${transaction.hash}`
          );
        });
      }
    }
