        ...
        if (eventArg === undefined) {
          ...
          try {
            ...
            if (arg.rescue) {
              ...
              // `Gate (RToken Eth Address) (... deploy rToken)`
              // could be used to deploy a rToken if it doesn't exist, but
              // since there is no RToken, that check would raise (when we'd
