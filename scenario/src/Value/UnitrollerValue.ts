...
async function getCointrollerImplementation(world: World, unitroller: Unitroller): Promise<AddressV> {
  return new AddressV(await unitroller.methods.cointrollerImplementation().call());
}

async function getPendingCointrollerImplementation(world: World, unitroller: Unitroller): Promise<AddressV> {
  return new AddressV(await unitroller.methods.pendingCointrollerImplementation().call());
}
    ...
    new Fetcher<{ unitroller: Unitroller }, AddressV>(
      `
        ...
        * "Unitroller Implementation" - Returns the Implementation of Unitroller contract
          * E.g. "Unitroller Implementation" - Returns address of cointrollerImplentation
      `,
      'Implementation',
      [new Arg('unitroller', getUnitroller, { implicit: true })],
      (world, { unitroller }) => getCointrollerImplementation(world, unitroller)
    ),
    new Fetcher<{ unitroller: Unitroller }, AddressV>(
      `
        ...
        * "Unitroller PendingImplementation" - Returns the pending implementation of Unitroller contract
          * E.g. "Unitroller PendingImplementation" - Returns address of pendingCointrollerImplementation
      `,
      'PendingImplementation',
      [new Arg('unitroller', getUnitroller, { implicit: true })],
      (world, { unitroller }) => getPendingCointrollerImplementation(world, unitroller)
    )
  ];
}
