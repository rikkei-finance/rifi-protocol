...
const JumpRateModel = getContract('JumpRateModel');
const LegacyJumpRateModel = getContract('LegacyJumpRateModel');
...
export async function buildInterestRateModel(world: World, from: string, event: Event): Promise<{world: World, interestRateModel: InterestRateModel, interestRateModelData: InterestRateModelData}> {
  const fetchers = [
    ...
    new Fetcher<{name: StringV, baseRate: NumberV, multiplier: NumberV, jump: NumberV, kink: NumberV, owner: AddressV}, InterestRateModelData>(`
         ...
         * "JumpRateModel name:<String> baseRate:<Number> multiplier:<Number> jump:<Number> kink:<Number> owner:<Address>" - The v2 Jump interest rate model
           * E.g. "InterestRateModel Deploy JumpRateModel MyInterestRateModel 0.05 0.2 2 0.90 (Address Timelock)" - 5% base rate and 20% + 5% interest at kink and 200% multiplier starting at the kink of 90% utilization
       `,
       "JumpRateModel",
       [
         new Arg("name", getStringV),
       ...
       async (world, {name, baseRate, multiplier, jump, kink, owner}) => ({
         invokation: await JumpRateModel.deploy<InterestRateModel>(world, from, [baseRate.encode(), multiplier.encode(), jump.encode(), kink.encode(), owner.val]),
         name: name.val,
         contract: "JumpRateModel",
         description: `JumpRateModel baseRate=${baseRate.encode().toString()} multiplier=${multiplier.encode().toString()} jump=${jump.encode().toString()} kink=${kink.encode().toString()}`,
         base: baseRate.encode().toString(),
    ...
    new Fetcher<{name: StringV, baseRate: NumberV, multiplier: NumberV, jump: NumberV, kink: NumberV, owner: AddressV}, InterestRateModelData>(`
         ...
         * "LegacyJumpRateModel name:<String> baseRate:<Number> multiplier:<Number> jump:<Number> kink:<Number> owner:<Address>" - The legacy v2 Jump interest rate model
           * E.g. "InterestRateModel Deploy LegacyJumpRateModel MyInterestRateModel 0.05 0.2 2 0.90 (Address Timelock)" - 5% base rate and 20% + 5% interest at kink and 200% multiplier starting at the kink of 90% utilization
       `,
       "LegacyJumpRateModel",
       [
         new Arg("name", getStringV),
       ...
       async (world, {name, baseRate, multiplier, jump, kink, owner}) => ({
         invokation: await LegacyJumpRateModel.deploy<InterestRateModel>(world, from, [baseRate.encode(), multiplier.encode(), jump.encode(), kink.encode(), owner.val]),
         name: name.val,
         contract: "LegacyJumpRateModel",
         description: `LegacyJumpRateModel baseRate=${baseRate.encode().toString()} multiplier=${multiplier.encode().toString()} jump=${jump.encode().toString()} kink=${kink.encode().toString()}`,
         base: baseRate.encode().toString(),
         slope: multiplier.encode().toString(),
         jump: jump.encode().toString(),
         kink: kink.encode().toString(),
