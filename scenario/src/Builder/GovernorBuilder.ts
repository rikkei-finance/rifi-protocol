...
export async function buildGovernor(
  ...
  const fetchers = [
    new Fetcher<
      { name: StringV, timelock: AddressV, rifi: AddressV, guardian: AddressV },
      GovernorData
    >(
      `
      #### GovernorAlpha

      * "Governor Deploy Alpha name:<String> timelock:<Address> rifi:<Address> guardian:<Address>" - Deploys Rifi Governor Alpha
        * E.g. "Governor Deploy Alpha GovernorAlpha (Address Timelock) (Address Rifi) Guardian"
    `,
      "Alpha",
      [
        new Arg("name", getStringV),
        new Arg("timelock", getAddressV),
        new Arg("rifi", getAddressV),
        new Arg("guardian", getAddressV)
      ],
      async (world, { name, timelock, rifi, guardian }) => {
        return {
          invokation: await GovernorAlphaContract.deploy<Governor>(
            world,
            from,
            [timelock.val, rifi.val, guardian.val]
          ),
          name: name.val,
    ...
    >(
      ...
      * "Governor Deploy AlphaHarness name:<String> timelock:<Address> rifi:<Address> guardian:<Address>" - Deploys Rifi Governor Alpha with a mocked voting period
        * E.g. "Governor Deploy AlphaHarness GovernorAlphaHarness (Address Timelock) (Address Rifi) Guardian"
    `,
      "AlphaHarness",
      [
        new Arg("name", getStringV),
