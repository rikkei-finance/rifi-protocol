...
import { cointrollerFetchers, getCointrollerValue } from './Value/CointrollerValue';
import { cointrollerImplFetchers, getCointrollerImplValue } from './Value/CointrollerImplValue';
import { getUnitrollerValue, unitrollerFetchers } from './Value/UnitrollerValue';
import { rTokenFetchers, getRTokenValue } from './Value/RTokenValue';
import { rTokenDelegateFetchers, getRTokenDelegateValue } from './Value/RTokenDelegateValue';
...
import { getRifiValue, rifiFetchers } from './Value/RifiValue';
...
const fetchers = [
    ...
    async (world, { addr, slot, key, nestedKey, valType }) => {
      ...
      switch (valType.val) {
        case 'marketStruct':
          ...
          let isRifiKey = '0x' + toBN(newKey).add(toBN(3)).toString(16);
          let isRifiStr = await world.web3.eth.getStorageAt(addr.val, isRifiKey);

          return new ListV([
            new BoolV(isListed),
            new ExpNumberV(collateralFactor.toString(), 1e18),
            new BoolV(areEqual(userInMarket, 1)),
            new BoolV(areEqual(isRifiStr, 1))
          ]);
        default:
          return new NothingV();
      }
  ...
  new Fetcher<{ given: Value; expected: Value }, BoolV>(
    `
      ...
      * "Equal given:<Value> expected:<Value>" - Returns true if given values are equal
        ...
        * E.g. "Equal (RToken rZRX TotalSupply) (Exactly 55)"
        * E.g. "Equal (RToken rZRX Cointroller) (Cointroller Address)"
    `,
    'Equal',
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "Cointroller ...cointrollerArgs" - Returns cointroller value
    `,
    'Cointroller',
    [new Arg('res', getCointrollerValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: cointrollerFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### CointrollerImpl

      * "CointrollerImpl ...cointrollerImplArgs" - Returns cointroller implementation value
    `,
    'CointrollerImpl',
    [new Arg('res', getCointrollerImplValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: cointrollerImplFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### RToken

      * "RToken ...rTokenArgs" - Returns rToken value
    `,
    'RToken',
    [new Arg('res', getRTokenValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: rTokenFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### RTokenDelegate

      * "RTokenDelegate ...rTokenDelegateArgs" - Returns rToken delegate value
    `,
    'RTokenDelegate',
    [new Arg('res', getRTokenDelegateValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: rTokenDelegateFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### Bep20
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "Rifi ...rifiArgs" - Returns Rifi value
    `,
    'Rifi',
    [new Arg('res', getRifiValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: rifiFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### Governor
...
let contractFetchers = [
  ...
  { contract: "RifiLens", implicit: false },
