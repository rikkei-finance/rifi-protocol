...
import { cointrollerCommands, processCointrollerEvent } from './Event/CointrollerEvent';
import { processUnitrollerEvent, unitrollerCommands } from './Event/UnitrollerEvent';
import { cointrollerImplCommands, processCointrollerImplEvent } from './Event/CointrollerImplEvent';
import { rTokenCommands, processRTokenEvent } from './Event/RTokenEvent';
import { rTokenDelegateCommands, processRTokenDelegateEvent } from './Event/RTokenDelegateEvent';
...
import { rifiCommands, processRifiEvent } from './Event/RifiEvent';
...
import { RifiLens } from './Contract/RifiLens';
...
export const commands: (View<any> | ((world: World) => Promise<View<any>>))[] = [
  ...
  new Command<{ blockNumber: NumberV, event: EventV }>(
    `
      ...
      * "Block 10 (...event)" - Set block to block N and run event
        * E.g. "Block 10 (Rifi Deploy Admin)"
    `,
    'Block',
    [
      new Arg('blockNumber', getNumberV),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Trx ...trxEvent" - Handles event to set details of next transaction
        * E.g. "Trx Value 1.0e18 (RToken cEth Mint 1.0e18)"
    `,
    'Trx',
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Expect ...expectation" - Adds an expectation to hold after the next transaction
        * E.g. "Expect Changes (RToken rZRX TotalSupply) +10.0e18"
    `,
    'Expect',
    [new Arg('event', getEventV, { variadic: true })],
  ...
  new Command<{ given: Value; event: EventV }>(
    `
      ...
      * "Given value event" - Runs event only if value is truthy. Thus, given can be used to build existence checks.
        * E.g. "Given ($var) (PriceOracle SetPrice rBAT $var)"
    `,
    'Given',
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Unitroller ...event" - Runs given Unitroller event
        * E.g. "Unitroller SetPendingImpl MyCointrollerImpl"
    `,
    'Unitroller',
    [new Arg('event', getEventV, { variadic: true })],
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Cointroller ...event" - Runs given Cointroller event
        * E.g. "Cointroller _setReserveFactor 0.5"
    `,
    'Cointroller',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processCointrollerEvent(world, event.val, from),
    { subExpressions: cointrollerCommands() }
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "CointrollerImpl ...event" - Runs given CointrollerImpl event
        * E.g. "CointrollerImpl MyImpl Become"
    `,
    'CointrollerImpl',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processCointrollerImplEvent(world, event.val, from),
    { subExpressions: cointrollerImplCommands() }
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "RToken ...event" - Runs given RToken event
        * E.g. "RToken rZRX Mint 5e18"
    `,
    'RToken',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processRTokenEvent(world, event.val, from),
    { subExpressions: rTokenCommands() }
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "RTokenDelegate ...event" - Runs given RTokenDelegate event
        * E.g. "RTokenDelegate Deploy RDaiDelegate rDaiDelegate"
    `,
    'RTokenDelegate',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processRTokenDelegateEvent(world, event.val, from),
    { subExpressions: rTokenDelegateCommands() }
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * E.g. "PriceOracleProxy Deploy (Unitroller Address) (PriceOracle Address) (RToken rETH Address)"
    `,
    'PriceOracleProxy',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * E.g. "Maximillion Deploy (RToken rETH Address)"
    `,
    'Maximillion',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Rifi ...event" - Runs given rifi event
      * E.g. "Rifi Deploy"
    `,
    'Rifi',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
      return processRifiEvent(world, event.val, from);
    },
    { subExpressions: rifiCommands() }
  ),
  ...
  buildContractEvent<RifiLens>("RifiLens", false),
