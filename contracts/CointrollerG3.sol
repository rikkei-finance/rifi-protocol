 ...
 * @author Rifi
 */
contract CointrollerG3 is CointrollerV3Storage, CointrollerInterface, CointrollerErrorReporter, Exponential {
    ...
    function _become(Unitroller unitroller, uint rifiRate_, address[] memory rifiMarketsToAdd, address[] memory otherMarketsToAdd) public {
        require(msg.sender == unitroller.admin(), "only unitroller admin can change brains");
        require(unitroller._acceptImplementation() == 0, "change not authorized");

        CointrollerG3(address(unitroller))._becomeG3(rifiRate_, rifiMarketsToAdd, otherMarketsToAdd);
    }

    function _becomeG3(uint rifiRate_, address[] memory rifiMarketsToAdd, address[] memory otherMarketsToAdd) public {
        require(msg.sender == cointrollerImplementation, "only brains can become itself");

        for (uint i = 0; i < rifiMarketsToAdd.length; i++) {
            _addMarketInternal(address(rifiMarketsToAdd[i]));
        }
        ...
        _setRifiRate(rifiRate_);
        _addRifiMarkets(rifiMarketsToAdd);
    }
    ...
    function refreshRifiSpeeds() public {
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            ...
            if (markets[address(rToken)].isRified) {
                ...
                Exp memory interestPerBlock = mul_(Exp({mantissa: rToken.borrowRatePerBlock()}), rToken.totalBorrows());
