...
contract GovernorBravoDelegate is GovernorBravoDelegateStorageV2, GovernorBravoEvents {
    ...
    string public constant name = "Rifi Governor Bravo";

    /// @notice The minimum setable proposal threshold
    uint public constant MIN_PROPOSAL_THRESHOLD = 50000e18; // 50,000 Rifi

    /// @notice The maximum setable proposal threshold
    uint public constant MAX_PROPOSAL_THRESHOLD = 100000e18; //100,000 Rifi
    ...
    uint public constant quorumVotes = 400000e18; // 400,000 = 4% of Rifi
    ...
    function initialize(address timelock_, address rifi_, uint votingPeriod_, uint votingDelay_, uint proposalThreshold_) public {
        ...
        require(rifi_ != address(0), "GovernorBravo::initialize: invalid rifi address");
    ...
    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        ...
        require(rifi.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold || isWhitelisted(msg.sender), "GovernorBravo::propose: proposer votes below proposal threshold");
    ...
    function cancel(uint proposalId) external {
        ...
        if(msg.sender != proposal.proposer) {
            ...
            if(isWhitelisted(proposal.proposer)) {
                require((rifi.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold) && msg.sender == whitelistGuardian, "GovernorBravo::cancel: whitelisted proposer");
            }
            else {
