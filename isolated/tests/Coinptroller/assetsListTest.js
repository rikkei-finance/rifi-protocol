...
describe('assetListTest', () => {
  ...
  beforeEach(async () => {
    ...
    cointroller = await makeCointroller({maxAssets: 10});
    allTokens = [OMG, ZRX, BAT, REP, DAI, SKT] = await Promise.all(
      ['OMG', 'ZRX', 'BAT', 'REP', 'DAI', 'sketch']
        .map(async (name) => makeRToken({cointroller, name, symbol: name, supportMarket: name != 'sketch', underlyingPrice: 0.5}))
    );
  });
  ...
  async function checkMarkets(expectedTokens) {
    for (let token of allTokens) {
      ...
      expect(await call(cointroller, 'checkMembership', [customer, token._address])).toEqual(isExpected);
    }
  }

  async function enterAndCheckMarkets(enterTokens, expectedTokens, expectedErrors = null) {
    const {reply, receipt} = await both(cointroller, 'enterMarkets', [enterTokens.map(t => t._address)], {from: customer});
    const assetsIn = await call(cointroller, 'getAssetsIn', [customer]);
  ...
  async function exitAndCheckMarkets(exitToken, expectedTokens, expectedError = 'NO_ERROR') {
    const {reply, receipt} = await both(cointroller, 'exitMarket', [exitToken._address], {from: customer});
    const assetsIn = await call(cointroller, 'getAssetsIn', [customer]);
  ...
  describe('enterMarkets', () => {
    ...
    it("the market must be listed for add to succeed", async () => {
      ...
      await send(cointroller, '_supportMarket', [SKT._address]);
  ...
  describe('entering from borrowAllowed', () => {
    it("enters when called by a rtoken", async () => {
    ...
    it("reverts when called by not a rtoken", async () => {
      await expect(
        send(cointroller, 'borrowAllowed', [BAT._address, customer, 1], {from: customer})
      ).rejects.toRevert('revert sender must be rToken');
