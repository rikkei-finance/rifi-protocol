...
export async function buildMaximillion(world: World, from: string, event: Event): Promise<{world: World, maximillion: Maximillion, maximillionData: MaximillionData}> {
  const fetchers = [
    new Fetcher<{rBinance: AddressV}, MaximillionData>(`
        #### Maximillion
      ...
      "Maximillion",
      [
        new Arg("rBinance", getAddressV)
      ],
      async (world, {rBinance}) => {
        return {
          invokation: await MaximillionContract.deploy<Maximillion>(world, from, [rBinance.val]),
          description: "Maximillion",
          rBinanceAddress: rBinance.val
        };
      },
      {catchall: true}
    )
