...
import { getCointrollerImpl, getUnitroller } from '../ContractLookup';
...
async function acceptAdmin(world: World, from: string, unitroller: Unitroller): Promise<World> {
  let invokation = await invoke(world, unitroller.methods._acceptAdmin(), from, CointrollerErrorReporter);
...
async function setPendingImpl(
  ...
  let invokation = await invoke(
    world,
    unitroller.methods._setPendingImplementation(cointrollerImpl._address),
    from,
    CointrollerErrorReporter
  );

  world = addAction(world, `Set pending cointroller impl to ${cointrollerImpl.name}`, invokation);
  ...
  return [
    ...
    new Command<{ unitroller: Unitroller; cointrollerImpl: CointrollerImpl }>(
      `
        ...
        * "SetPendingImpl impl:<Impl>" - Sets the pending cointroller implementation for this unitroller
          * E.g. "Unitroller SetPendingImpl MyScenImpl" - Sets the current cointroller implementation to MyScenImpl
      `,
      'SetPendingImpl',
      [
        new Arg('unitroller', getUnitroller, { implicit: true }),
      ...
      (world, from, { unitroller, cointrollerImpl }) =>
        setPendingImpl(world, from, unitroller, cointrollerImpl)
    )
  ];
}
