...
async function setPrice(world: World, from: string, priceOracle: PriceOracle, rToken: string, amount: NumberV): Promise<World> {
  return addAction(
    world,
    `Set price oracle price for ${rToken} to ${amount.show()}`,
    await invoke(world, priceOracle.methods.setUnderlyingPrice(rToken, amount.encode()), from)
  );
}
...
export function priceOracleCommands() {
  return [
    ...
    new Command<{priceOracle: PriceOracle, rToken: AddressV, amount: NumberV}>(`
        #### SetPrice
      ...
      [
        ...
        new Arg("rToken", getAddressV),
        new Arg("amount", getExpNumberV)
      ],
      (world, from, {priceOracle, rToken, amount}) => setPrice(world, from, priceOracle, rToken.val, amount)
    ),

    new Command<{priceOracle: PriceOracle, address: AddressV, amount: NumberV}>(`
        #### SetDirectPrice

        * "SetDirectPrice <Address> <Amount>" - Sets the per-ether price for the given rToken
