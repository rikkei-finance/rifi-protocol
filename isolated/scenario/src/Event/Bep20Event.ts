...
async function approve(world: World, from: string, bep20: Bep20, address: string, amount: NumberV): Promise<World> {
  let invokation = await invoke(world, bep20.methods.approve(address, amount.encode()), from, RTokenErrorReporter);
...
async function faucet(world: World, from: string, bep20: Bep20, address: string, amount: NumberV): Promise<World> {
  let invokation = await invoke(world, bep20.methods.allocateTo(address, amount.encode()), from, RTokenErrorReporter);
...
async function transfer(world: World, from: string, bep20: Bep20, address: string, amount: NumberV): Promise<World> {
  let invokation = await invoke(world, bep20.methods.transfer(address, amount.encode()), from, RTokenErrorReporter);
...
async function transferFrom(world: World, from: string, bep20: Bep20, owner: string, spender: string, amount: NumberV): Promise<World> {
  let invokation = await invoke(world, bep20.methods.transferFrom(owner, spender, amount.encode()), from, RTokenErrorReporter);
...
async function setFail(world: World, from: string, bep20: Bep20, fail: boolean): Promise<World> {
  let invokation = await invoke(world, bep20.methods.setFail(fail), from, RTokenErrorReporter);
...
export function bep20Commands() {
  return [
    ...
    new View<{bep20Arg: StringV, apiKey: StringV}>(`
        ...
        * "Bep20 <bep20> Verify apiKey:<String>" - Verifies Bep20 in Etherscan
    ...
    new Command<{bep20: Bep20, spender: AddressV, amount: NumberV}>(`
        ...
        * "Bep20 <Bep20> Approve spender:<Address> <Amount>" - Adds an allowance between user and address
          * E.g. "Bep20 ZRX Approve rZRX 1.0e18"
      `,
      "Approve",
      [
