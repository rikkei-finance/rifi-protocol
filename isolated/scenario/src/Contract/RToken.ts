...
export interface RTokenMethods {
  ...
  liquidateBorrow(borrower: string, rTokenCollateral: string): Sendable<number>;
  liquidateBorrow(borrower: string, repayAmount: encodedNumber, rTokenCollateral: string): Sendable<number>;
  ...
  _setCointroller(address: string): Sendable<number>;
...
export interface RTokenScenarioMethods extends RTokenMethods {
  setTotalBorrows(amount: encodedNumber): Sendable<void>;
  setTotalReserves(amount: encodedNumber): Sendable<void>;
}

export interface RToken extends Contract {
  methods: RTokenMethods;
  name: string;
}

export interface RTokenScenario extends Contract {
  methods: RTokenScenarioMethods;
  name: string;
}
