...
(async function() {
  ...
  console.log(`Deploying rToken with ${JSON.stringify(conf)}`);
  ...
  let contract = await saddle.deploy('RBep20Immutable', deployArgs);
  ...
  if (env['VERIFY']) {
    ...
    await saddle.verify(etherscanApiKey, contract._address, 'RBep20Immutable', deployArgs, 0);
