...
usage:
	npx saddle script -n {network} flywheel:init {
		batch: number || null, // how many borrowrs to claim rifi for at once. default 100 (~2.3M gas)
		stage: bool || null // use stage API? default false
		rTokens: string[] || null, // which borrowers to get. defaults to all rTokens
...
let getConfig = (configArgs) => {
	...
	let res = {
		rTokens: getArray(config, 'rTokens', false) || [],
...
let getRTokenAddresses = (rTokenArgs) => {
	let all = [
		'rUSDC',
		'rDAI',
		'rUSDT',
	...
	let list = rTokenArgs.length == 0 ? all : rTokenArgs;
	let map = {};
	for(let val of list) {
...
let accountRequest = async (network, opts) => {
	...
	let url = `https://api.${stageUrl}rikkei.finance/api/v2/account?min_borrow_value_in_eth[value]=0.00000000000000001&network=${network}&page_size=${pageSize}&page_number=1`;
	let res = await fetch(url);
	return JSON.parse(res).accounts;
};

let filterInitialized = async (borrowersByRToken) => {
	let res = {}
	let batchSize = 75;
	console.log(`Calling rifiBorrowerIndex for borrowers in batches of ${batchSize}...\n`);
	for(let rTokenAddr of Object.keys(borrowersByRToken)) {
		let speed = await call(Cointroller, 'rifiSpeeds', [rTokenAddr]);
		if (Number(speed) != 0){
			for (let borrowerChunk of getChunks(borrowersByRToken[rTokenAddr], batchSize)) {
				try {
					let indices = await Promise.all(borrowerChunk.map(
						async(borrower) => {
							return await call(Cointroller, 'rifiBorrowerIndex',[rTokenAddr, borrower])
					}));
					let uninitialized = borrowerChunk.filter((borrower, i) => Number(indices[i]) == 0);
					res[rTokenAddr] = res[rTokenAddr] ? res[rTokenAddr].concat(uninitialized) : uninitialized;
				} catch(e) {
...
// {[rtokenAddr] : borrowers}
let filterBorrowers = (apiAccounts, rTokenList) => {
	return apiAccounts.reduce((acc, account) => {
		let validBorrowers = account.tokens.filter(
			(accountRToken) =>
				rTokenList.includes(accountRToken.address) &&
				accountRToken.borrow_balance_underlying.value > 0
		);
		for (let borrower of validBorrowers) {
			let rtokenAddr = borrower.address;
			acc[rtokenAddr] = acc[rtokenAddr]
				? acc[rtokenAddr].concat(account.address)
				: [account.address];
		}
		return acc;
	}, {});
...
let claimRifiBatch = async (borrowersByRToken, opts) => {
	for (let rTokenAddr of Object.keys(borrowersByRToken)) {
		let borrowers = borrowersByRToken[rTokenAddr];
		for (let chunk of getChunks(borrowers, opts.batch)) {
			if (chunk.length == 0) {
				console.log(`No borrowers to claim for ${rTokenAddr}`);
			} else {
				console.log(
					`Sending tx to claim ${rTokenAddr.toString()} borrows for ${JSON.stringify(
						chunk
					)}\n`
				);
				try {
					let tx = await send(Cointroller, 'claimRifi', [
						chunk,
						[rTokenAddr],
						true,
						false,
...
(async () => {
	let borrowersByRToken;
	let rTokenMap; // symbol => addrs
	let opts = getConfig(args[0]);
	if (network == 'development') {
		borrowersByRToken = getTestData();
	} else if (isKnownNetwork(network)) {
		let apiAccounts = opts.readFixture
			? await readFixture()
			: await accountRequest(network, opts);
		let rTokenAddresses = Object.values(getRTokenAddresses(opts.rTokens));
		borrowersByRToken = filterBorrowers(apiAccounts, rTokenAddresses);
		if (opts.writeFixture) await writeFixture(apiAccounts);
	} else {
		printUsage();
	}
	let unInit = await filterInitialized(borrowersByRToken);
	print('Uninitialized accounts before: ', unInit);

	await claimRifiBatch(unInit, opts);
	unInit = await filterInitialized(borrowersByRToken);
