...
contract('Rifi Test', function (accounts) {
  ...
  describe('Test common flow', async function () {
    ...
    const saveAddresses = async () => (fs.promises.writeFile(addressOutput, JSON.stringify(addresses)));
    ...
    beforeEach(async () => {
      ...
      if (tokens.length > 0) {
        for (let [, token] of tokens) {
          ...
          tokenDecimals[underlying.symbol] = underlying.decimals;
