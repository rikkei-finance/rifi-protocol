...
describe.skip('RifiWheelFuzzTest', () => {
  // This whole test is fake, but we're testing to see if our equations match reality.

  // First, we're going to build a simple simulator of the Rifi protocol
  ...
  let initialState = globals => {
    return {
      ...
      // flywheel & cointroller
      rifiSupplySpeed: new bn(1),
      rifiSupplyIndex: new bn(1),
      rifiSupplyIndexSnapshots: {},
      rifiSupplyIndexUpdatedBlock: globals.blockNumber,

      rifiBorrowSpeed: new bn(1),
      rifiBorrowIndex: new bn(1),
      rifiBorrowIndexSnapshots: {},
      rifiSupplyIndexUpdatedBlock: globals.blockNumber,

      rifiAccruedWithCrank: {}, // naive method, accruing all accounts every block
      rifiAccruedWithIndex: {}, // with indices

      activeBorrowBlocks: new bn(0), // # blocks with an active borrow, for which we expect to see rifi distributed. just for fuzz testing.
  ...
  let trueUpRifi = (globals, state) => {
    state = accrueInterest(globals, state);

    state = Object.keys(state.rifiSupplyIndexSnapshots).reduce(
      (acc, account) => supplierFlywheelByIndex(globals, state, account),
      state
    );

    state = Object.keys(state.rifiBorrowIndexSnapshots).reduce(
  ...
  // manual flywheel loops through every account and updates rifi accrued mapping
  ...
  let flywheelByCrank = (
    ...
    for (let [account, balance] of Object.entries(balances)) {
      if (isPositive(totalSupply)) {
        rifiAccruedWithCrank[account] = get(
          state.rifiAccruedWithCrank[account]
        ).plus(
          deltaBlocks
            .times(rifiSupplySpeed)
            .times(balance)
            .div(totalSupply)
        );
      }
    ...
    return {
      ...state,
      rifiAccruedWithCrank: rifiAccruedWithCrank,
    };
  };

  // real deal rifi index flywheel™️
  let borrowerFlywheelByIndex = (globals, state, account) => {
    let {
      rifiBorrowSpeed,
      rifiBorrowIndex,
      rifiBorrowIndexSnapshots,
      rifiAccruedWithIndex,
      totalBorrows,
      borrowBalances,
      rifiBorrowIndexUpdatedBlock,
      borrowIndex,
      borrowIndexSnapshots
    } = state;

    let deltaBlocks = globals.blockNumber.minus(rifiBorrowIndexUpdatedBlock);
    if (isPositive(totalBorrows)) {
      let scaledTotalBorrows = totalBorrows.div(borrowIndex);
      rifiBorrowIndex = rifiBorrowIndex.plus(
        rifiBorrowSpeed.times(deltaBlocks).div(scaledTotalBorrows)
      );
    }

    let indexSnapshot = rifiBorrowIndexSnapshots[account];

    if (
      indexSnapshot !== undefined &&
      rifiBorrowIndex.isGreaterThan(indexSnapshot) &&
      ...
      rifiAccruedWithIndex[account] = get(rifiAccruedWithIndex[account]).plus(
        borrowBalanceNew
          .div(borrowIndex)
          .times(rifiBorrowIndex.minus(indexSnapshot))
      );
    }

    return {
      ...state,
      rifiBorrowIndexUpdatedBlock: globals.blockNumber,
      rifiBorrowIndex: rifiBorrowIndex,
      rifiBorrowIndexSnapshots: {
        ...state.rifiBorrowIndexSnapshots,
        [account]: rifiBorrowIndex
      }
    };
  };
  ...
  let supplierFlywheelByIndex = (globals, state, account) => {
    let {
      ...
      rifiAccruedWithIndex,
      totalSupply,
      rifiSupplyIndexUpdatedBlock
    } = state;

    let deltaBlocks = globals.blockNumber.minus(rifiSupplyIndexUpdatedBlock);

    if (isPositive(totalSupply)) {
      rifiSupplyIndex = rifiSupplyIndex.plus(
        rifiSupplySpeed.times(deltaBlocks).div(totalSupply)
      );
    }

    let indexSnapshot = rifiSupplyIndexSnapshots[account];
    if (indexSnapshot !== undefined) {
      // if had prev snapshot,  accrue some comp
      rifiAccruedWithIndex[account] = get(rifiAccruedWithIndex[account]).plus(
        balances[account].times(rifiSupplyIndex.minus(indexSnapshot))
      );
    }
    ...
    return {
      ...state,
      rifiSupplyIndexUpdatedBlock: globals.blockNumber,
      rifiSupplyIndex: rifiSupplyIndex,
      rifiSupplyIndexSnapshots: {
        ...state.rifiSupplyIndexSnapshots,
        [account]: rifiSupplyIndex
      },
      rifiAccruedWithIndex: rifiAccruedWithIndex
    };
  };
  ...
  // assert amount distributed by the crank is expected, that it equals # blocks with a supply * rifi speed
  let crankCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.activeSupplyBlocks
      .times(state.rifiSupplySpeed)
      .plus(state.activeBorrowBlocks.times(state.rifiBorrowSpeed));

    let actual = Object.values(state.rifiAccruedWithCrank).reduce(
    ...
    invariantFn(
      ...
      expected,
      actual,
      `crank method distributed rifi inaccurately`
    );
  };

  // assert rifi distributed by index is the same as amount distributed by crank
  let indexCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.rifiAccruedWithCrank;
    let actual = state.rifiAccruedWithIndex;
    invariantFn(
      (expected, actual) => {
  ...
  let runEvents = (globals, initState, events) => {
    ...
    return trueUpRifi(globals, state);
  };
