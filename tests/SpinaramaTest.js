...
describe('Spinarama', () => {
  ...
  describe('#mintMint', () => {
    it('should succeed', async () => {
      ...
      await send(rToken.underlying, 'harnessSetBalance', [from, 100], {from});
      await send(rToken.underlying, 'approve', [rToken._address, UInt256Max()], {from});
      await minerStop();
      const p1 = send(rToken, 'mint', [1], {from});
      const p2 = send(rToken, 'mint', [2], {from});
      await minerStart();
      expect(await p1).toSucceed();
      expect(await p2).toSucceed();
      expect(await balanceOf(rToken, from)).toEqualNumber(3);
    });

    it('should partial succeed', async () => {
      const rToken = await makeRToken({supportMarket: true});
      await send(rToken.underlying, 'harnessSetBalance', [from, 100], {from});
      await send(rToken.underlying, 'approve', [rToken._address, 10], {from});
      await minerStop();
      const p1 = send(rToken, 'mint', [11], {from});
      const p2 = send(rToken, 'mint', [10], {from});
      ...
      expect(await balanceOf(rToken, from)).toEqualNumber(10);
    });
  });
  ...
  describe('#mintRedeem', () => {
    it('should succeed', async () => {
      ...
      await send(rToken.underlying, 'approve', [rToken._address, 10], {from});
      await minerStop();
      const p1 = send(rToken, 'mint', [10], {from});
      const p2 = send(rToken, 'redeemUnderlying', [10], {from});
      await minerStart();
      expect(await p1).toSucceed();
      expect(await p2).toSucceed();
      expect(await balanceOf(rToken, from)).toEqualNumber(0);
    });
  });
  ...
  describe('#redeemMint', () => {
    it('should succeed', async () => {
      const rToken = await makeRToken({supportMarket: true});
      await send(rToken, 'harnessSetTotalSupply', [10]);
      await send(rToken, 'harnessSetExchangeRate', [etherMantissa(1)]);
      await send(rToken, 'harnessSetBalance', [from, 10]);
      await send(rToken.underlying, 'harnessSetBalance', [rToken._address, 10]);
      await send(rToken.underlying, 'approve', [rToken._address, 10], {from});
      await minerStop();
      const p1 = send(rToken, 'redeem', [10], {from});
      const p2 = send(rToken, 'mint', [10], {from});
      await minerStart();
      expect(await p1).toSucceed();
      expect(await p2).toSucceed();
  ...
  describe('#repayRepay', () => {
    it('should succeed', async () => {
      const rToken1 = await makeRToken({supportMarket: true, underlyingPrice: 1, collateralFactor: .5});
      const rToken2 = await makeRToken({supportMarket: true, underlyingPrice: 1, cointroller: rToken1.cointroller});
      await send(rToken1.underlying, 'harnessSetBalance', [from, 10]);
      await send(rToken1.underlying, 'approve', [rToken1._address, 10], {from});
      await send(rToken2.underlying, 'harnessSetBalance', [rToken2._address, 10]);
      await send(rToken2, 'harnessSetTotalSupply', [100]);
      await send(rToken2.underlying, 'approve', [rToken2._address, 10], {from});
      await send(rToken2, 'harnessSetExchangeRate', [etherMantissa(1)]);
      expect(await enterMarkets([rToken1, rToken2], from)).toSucceed();
      expect(await send(rToken1, 'mint', [10], {from})).toSucceed();
      expect(await send(rToken2, 'borrow', [2], {from})).toSucceed();
      await minerStop();
      const p1 = send(rToken2, 'repayBorrow', [1], {from});
      const p2 = send(rToken2, 'repayBorrow', [1], {from});
      await minerStart();
      expect(await p1).toSucceed();
      expect(await p2).toSucceed();
      expect((await borrowSnapshot(rToken2, from)).principal).toEqualNumber(0);
    });
