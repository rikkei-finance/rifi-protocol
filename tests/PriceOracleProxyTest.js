...
describe('PriceOracleProxy', () => {
  ...
  beforeEach(async () => {
    ...
    cEth = await makeRToken({kind: "rbinance", cointrollerOpts: {kind: "v1-no-proxy"}, supportMarket: true});
    cUsdc = await makeRToken({cointroller: cEth.cointroller, supportMarket: true});
    cSai = await makeRToken({cointroller: cEth.cointroller, supportMarket: true});
    cDai = await makeRToken({cointroller: cEth.cointroller, supportMarket: true});
    cUsdt = await makeRToken({cointroller: cEth.cointroller, supportMarket: true});
    cOther = await makeRToken({cointroller: cEth.cointroller, supportMarket: true});
  ...
  describe("getUnderlyingPrice", () => {
    let setAndVerifyBackingPrice = async (rToken, price) => {
      await send(
        backingOracle,
        "setUnderlyingPrice",
        [rToken._address, etherMantissa(price)]);

      let backingOraclePrice = await call(
        backingOracle,
        "assetPrices",
        [rToken.underlying._address]);
    ...
    it("returns 0 for token without a price", async () => {
      let unlistedToken = await makeRToken({cointroller: cEth.cointroller});
