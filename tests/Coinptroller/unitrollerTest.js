...
describe('Unitroller', () => {
  ...
  describe("constructor", () => {
    it("sets admin to caller and addresses to 0", async () => {
      ...
      expect(await call(unitroller, 'pendingCointrollerImplementation')).toBeAddressZero();
      expect(await call(unitroller, 'cointrollerImplementation')).toBeAddressZero();
    });
  });
  ...
  describe("_setPendingImplementation", () => {
    ...
    describe("succeeding", () => {
      it("stores pendingCointrollerImplementation with value newPendingImplementation", async () => {
        await setPending(brains, root);
        expect(await call(unitroller, 'pendingCointrollerImplementation')).toEqual(brains._address);
      });
  ...
  describe("_acceptImplementation", () => {
    describe("Check caller is pendingCointrollerImplementation  and pendingCointrollerImplementation ≠ address(0) ", () => {
      let result;
      beforeEach(async () => {
      ...
      it("does not change current implementation address", async () => {
        expect(await call(unitroller, 'cointrollerImplementation')).not.toEqual(unitroller._address);
      });
    });
    ...
    describe("the brains must accept the responsibility of implementation", () => {
      ...
      it("Store cointrollerImplementation with value pendingCointrollerImplementation", async () => {
        expect(await call(unitroller, 'cointrollerImplementation')).toEqual(brains._address);
      });

      it("Unset pendingCointrollerImplementation", async () => {
        expect(await call(unitroller, 'pendingCointrollerImplementation')).toBeAddressZero();
      });
    ...
    describe("fallback delegates to brains", () => {
      let troll;
      beforeEach(async () => {
        troll = await deploy('EchoTypesCointroller');
        unitroller = await deploy('Unitroller');
