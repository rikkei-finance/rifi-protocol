const {
  etherMantissa,
  both
} = require('../Utils/Ethereum');

const {
  makeCointroller,
  makePriceOracle,
  makeRToken,
  makeToken
} = require('../Utils/Rifi');

describe('Cointroller', () => {
  let root, accounts;

  beforeEach(async () => {
    [root, ...accounts] = saddle.accounts;
  });

  describe('constructor', () => {
    it("on success it sets admin to creator and pendingAdmin is unset", async () => {
      const cointroller = await makeCointroller();
      expect(await call(cointroller, 'admin')).toEqual(root);
      expect(await call(cointroller, 'pendingAdmin')).toEqualNumber(0);
    });

    it("on success it sets closeFactor as specified", async () => {
      const cointroller = await makeCointroller();
      expect(await call(cointroller, 'closeFactorMantissa')).toEqualNumber(0.051e18);
    });
  });

  describe('_setLiquidationIncentive', () => {
    const initialIncentive = etherMantissa(1.0);
    const validIncentive = etherMantissa(1.1);
    const tooSmallIncentive = etherMantissa(0.99999);
    const tooLargeIncentive = etherMantissa(1.50000001);

    let cointroller;
    beforeEach(async () => {
      cointroller = await makeCointroller();
    });

    it("fails if called by non-admin", async () => {
      const {reply, receipt} = await both(cointroller, '_setLiquidationIncentive', [initialIncentive], {from: accounts[0]});
      expect(reply).toHaveTrollError('UNAUTHORIZED');
      expect(receipt).toHaveTrollFailure('UNAUTHORIZED', 'SET_LIQUIDATION_INCENTIVE_OWNER_CHECK');
      expect(await call(cointroller, 'liquidationIncentiveMantissa')).toEqualNumber(initialIncentive);
    });

    it("accepts a valid incentive and emits a NewLiquidationIncentive event", async () => {
      const {reply, receipt} = await both(cointroller, '_setLiquidationIncentive', [validIncentive]);
      expect(reply).toHaveTrollError('NO_ERROR');
      expect(receipt).toHaveLog('NewLiquidationIncentive', {
        oldLiquidationIncentiveMantissa: initialIncentive.toString(),
        newLiquidationIncentiveMantissa: validIncentive.toString()
      });
      expect(await call(cointroller, 'liquidationIncentiveMantissa')).toEqualNumber(validIncentive);
    });
  });

  describe('_setPriceOracle', () => {
    let cointroller, oldOracle, newOracle;
    beforeEach(async () => {
      cointroller = await makeCointroller();
      oldOracle = cointroller.priceOracle;
      newOracle = await makePriceOracle();
    });

    it("fails if called by non-admin", async () => {
      expect(
        await send(cointroller, '_setPriceOracle', [newOracle._address], {from: accounts[0]})
      ).toHaveTrollFailure('UNAUTHORIZED', 'SET_PRICE_ORACLE_OWNER_CHECK');
      expect(await cointroller.methods.oracle().call()).toEqual(oldOracle._address);
    });

    it.skip("reverts if passed a contract that doesn't implement isPriceOracle", async () => {
      await expect(send(cointroller, '_setPriceOracle', [cointroller._address])).rejects.toRevert();
      expect(await call(cointroller, 'oracle')).toEqual(oldOracle._address);
    });

    it.skip("reverts if passed a contract that implements isPriceOracle as false", async () => {
      await send(newOracle, 'setIsPriceOracle', [false]); // Note: not yet implemented
      await expect(send(notOracle, '_setPriceOracle', [cointroller._address])).rejects.toRevert("revert oracle method isPriceOracle returned false");
      expect(await call(cointroller, 'oracle')).toEqual(oldOracle._address);
    });

    it("accepts a valid price oracle and emits a NewPriceOracle event", async () => {
      const result = await send(cointroller, '_setPriceOracle', [newOracle._address]);
      expect(result).toSucceed();
      expect(result).toHaveLog('NewPriceOracle', {
        oldPriceOracle: oldOracle._address,
        newPriceOracle: newOracle._address
      });
      expect(await call(cointroller, 'oracle')).toEqual(newOracle._address);
    });
  });

  describe('_setCloseFactor', () => {
    it("fails if not called by admin", async () => {
      const rToken = await makeRToken();
      await expect(
        send(rToken.cointroller, '_setCloseFactor', [1], {from: accounts[0]})
      ).rejects.toRevert('revert only admin can set close factor');
    });
  });

  describe('_setCollateralFactor', () => {
    const half = etherMantissa(0.5);
    const one = etherMantissa(1);

    it("fails if not called by admin", async () => {
      const rToken = await makeRToken();
      expect(
        await send(rToken.cointroller, '_setCollateralFactor', [rToken._address, half], {from: accounts[0]})
      ).toHaveTrollFailure('UNAUTHORIZED', 'SET_COLLATERAL_FACTOR_OWNER_CHECK');
    });

    it("fails if asset is not listed", async () => {
      const rToken = await makeRToken();
      expect(
        await send(rToken.cointroller, '_setCollateralFactor', [rToken._address, half])
      ).toHaveTrollFailure('MARKET_NOT_LISTED', 'SET_COLLATERAL_FACTOR_NO_EXISTS');
    });

    it("fails if factor is set without an underlying price", async () => {
      const rToken = await makeRToken({supportMarket: true});
      expect(
        await send(rToken.cointroller, '_setCollateralFactor', [rToken._address, half])
      ).toHaveTrollFailure('PRICE_ERROR', 'SET_COLLATERAL_FACTOR_WITHOUT_PRICE');
    });

    it("succeeds and sets market", async () => {
      const rToken = await makeRToken({supportMarket: true, underlyingPrice: 1});
      const result = await send(rToken.cointroller, '_setCollateralFactor', [rToken._address, half]);
      expect(result).toHaveLog('NewCollateralFactor', {
        rToken: rToken._address,
        oldCollateralFactorMantissa: '0',
        newCollateralFactorMantissa: half.toString()
      });
    });
  });

  describe('_supportMarket', () => {
    it("fails if not called by admin", async () => {
      const rToken = await makeRToken(root);
      expect(
        await send(rToken.cointroller, '_supportMarket', [rToken._address], {from: accounts[0]})
      ).toHaveTrollFailure('UNAUTHORIZED', 'SUPPORT_MARKET_OWNER_CHECK');
    });

    it("fails if asset is not a RToken", async () => {
      const cointroller = await makeCointroller()
      const asset = await makeToken(root);
      await expect(send(cointroller, '_supportMarket', [asset._address])).rejects.toRevert();
    });

    it("succeeds and sets market", async () => {
      const rToken = await makeRToken();
      const result = await send(rToken.cointroller, '_supportMarket', [rToken._address]);
      expect(result).toHaveLog('MarketListed', {rToken: rToken._address});
    });

    it("cannot list a market a second time", async () => {
      const rToken = await makeRToken();
      const result1 = await send(rToken.cointroller, '_supportMarket', [rToken._address]);
      const result2 = await send(rToken.cointroller, '_supportMarket', [rToken._address]);
      expect(result1).toHaveLog('MarketListed', {rToken: rToken._address});
      expect(result2).toHaveTrollFailure('MARKET_ALREADY_LISTED', 'SUPPORT_MARKET_EXISTS');
    });

    it("can list two different markets", async () => {
      const rToken1 = await makeRToken();
      const rToken2 = await makeRToken({cointroller: rToken1.cointroller});
      const result1 = await send(rToken1.cointroller, '_supportMarket', [rToken1._address]);
      const result2 = await send(rToken1.cointroller, '_supportMarket', [rToken2._address]);
      expect(result1).toHaveLog('MarketListed', {rToken: rToken1._address});
      expect(result2).toHaveLog('MarketListed', {rToken: rToken2._address});
    });
  });

  describe('redeemVerify', () => {
    it('should allow you to redeem 0 underlying for 0 tokens', async () => {
      const cointroller = await makeCointroller();
      const rToken = await makeRToken({cointroller: cointroller});
      await call(cointroller, 'redeemVerify', [rToken._address, accounts[0], 0, 0]);
    });

    it('should allow you to redeem 5 underlyig for 5 tokens', async () => {
      const cointroller = await makeCointroller();
      const rToken = await makeRToken({cointroller: cointroller});
      await call(cointroller, 'redeemVerify', [rToken._address, accounts[0], 5, 5]);
    });

    it('should not allow you to redeem 5 underlying for 0 tokens', async () => {
      const cointroller = await makeCointroller();
      const rToken = await makeRToken({cointroller: cointroller});
      await expect(call(cointroller, 'redeemVerify', [rToken._address, accounts[0], 5, 0])).rejects.toRevert("revert redeemTokens zero");
    });
  })
});
