...
describe('RifiScenario', () => {
  let root, accounts;
  let rifi;

  beforeEach(async () => {
    [root, ...accounts] = saddle.accounts;
    rifi = await deploy('RifiScenario', [root]);
  });
    ...
    [
      ...
      it(`with ${checkpoints} checkpoints, has ${expectedReads} reads`, async () => {
        ...
        while (remaining > 0) {
          ...
          await rifi.methods.generateCheckpoints(amt, offset).send({from: root, gas: 200000000});
          remaining -= amt;
          offset += amt;
        }

        let result = await rifi.methods.getPriorVotes(root, 1).send();
