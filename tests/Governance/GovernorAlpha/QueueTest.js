...
describe('GovernorAlpha#queue/1', () => {
  ...
  describe("overlapping actions", () => {
    it("reverts on queueing overlapping actions in same proposal", async () => {
      ...
      const rifi = await deploy('Rifi', [root]);
      const gov = await deploy('GovernorAlpha', [timelock._address, rifi._address, root]);
      const txAdmin = await send(timelock, 'harnessSetAdmin', [gov._address]);

      await enfranchise(rifi, a1, 3e6);
      await mineBlock();

      const targets = [rifi._address, rifi._address];
      const values = ["0", "0"];
    ...
    it("reverts on queueing overlapping actions in different proposals, works if waiting", async () => {
      ...
      await enfranchise(rifi, a1, 3e6);
      await enfranchise(rifi, a2, 3e6);
      await mineBlock();

      const targets = [rifi._address];
      const values = ["0"];
