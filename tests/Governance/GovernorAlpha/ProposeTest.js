...
describe('GovernorAlpha#propose/5', () => {
  ...
  describe("simple initialization", () => {
    ...
    it("This function returns the id of the newly created proposal. # proposalId(n) = succ(proposalId(n-1))", async () => {
      await send(rifi, 'transfer', [accounts[2], etherMantissa(400001)]);
      await send(rifi, 'delegate', [accounts[2]], { from: accounts[2] });
    ...
    it("emits log with id and description", async () => {
      await send(rifi, 'transfer', [accounts[3], etherMantissa(400001)]);
      await send(rifi, 'delegate', [accounts[3]], { from: accounts[3] });
      await mineBlock();
