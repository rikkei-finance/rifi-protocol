...
describe('GovernorAlpha#state/1', () => {
  let rifi, gov, root, acct, delay, timelock;
  ...
  beforeAll(async () => {
    ...
    gov = await deploy('GovernorAlpha', [timelock._address, rifi._address, root]);
    await send(timelock, "harnessSetAdmin", [gov._address])
    await send(rifi, 'transfer', [acct, etherMantissa(4000000)]);
    await send(rifi, 'delegate', [acct], { from: acct });
  });
  ...
  it("Canceled", async () => {
    await send(rifi, 'transfer', [accounts[0], etherMantissa(4000000)]);
    await send(rifi, 'delegate', [accounts[0]], { from: accounts[0] });
    await mineBlock()
    ...
    await send(rifi, 'delegate', [root], { from: accounts[0] });
